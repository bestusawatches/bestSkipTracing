...
const LoginSection = () => {
  ...
  useEffect(() => {
    console.log("loginUserSuccess:", loginUserSuccessData)
    if (loginUserSuccessData?.data && loginClick) {
      setLSItem("user_data", JSON.stringify(loginUserSuccessData?.data));
      setLSItem("user_token", loginUserSuccessData?.token);
      dispatch(getSelectLanguage());
    }
  ...
  //           setLSItem("fluency_level", single?.fluency_level);
  //         }
  //       });
  //       navigate("/chat");
  //     }
  ...
  useEffect(() => {
    console.log("getSelectModelSuccessData:", getSelectModelSuccessData)
    if (getLSItem("user_type") === "1" && loginClick) {
  ...
  useEffect(() => {
    console.log("getSelectLanguageSuccessData", getSelectLanguageSuccessData);
    if (getSelectLanguageSuccessData && loginClick) {
      if (getSelectLanguageSuccessData?.data) {
        if (getLSItem("user_type") === "1") {
          setLSItem(
            "lang_id_agent",
            getSelectLanguageSuccessData?.data?.language
          );
          dispatch(getSelectModel());
        }
        if (getLSItem("user_type") === "2") {
          setLSItem(
            "lang_id_customer",
            getSelectLanguageSuccessData?.data?.language
          );
    ...
    <>{localStorage.setItem("login_status", "1")}
      <LoginSectionWrapper>
        <LoginFormWrapper>
          ...
          <LogoImage src="/logo.png" alt="Fish42" />
          <Title>Log in</Title>
