...
@router.post('/signup')
def sign_up(inp: Sign_up) -> dict[str, Any]:
    try:
        result = signup(inp)
        return result
...
def verify(inp: VerifyEmail) -> dict[str, Any]:
    try:
...
@router.post('/login')
def verify(inp: Login) -> dict[str, Any]:
    try:
        result = login(inp)
        return result
    except Exception as e:
...
def google_login(inp: GoogleLogin) -> dict[str, Any]:
    try:
...
@router.post('/logout')
def logout_user(authorization: str = Header(None))-> dict[str, Any]:
    try:
